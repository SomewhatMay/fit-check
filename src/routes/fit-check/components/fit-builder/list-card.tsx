...
export function ListCard({ item, index, setSelected, randomizeOthers }: props) {
  ...
  return (
    <button
      key={item.id}
      className="relative inline-block h-[12rem] aspect-square pt-[1rem] transition-all duration-400"
      style={{
        opacity: mounted ? 1 : 0,
    ...
    >
      ...
      {!imageLoaded ? (
        <div className="w-full bg-gray-500 animate-pulse">
          Image is loading!
        </div>
      ) : (
        <img
          src={item.imageId}
          alt="item"
          className="h-full aspect-square rounded-[2rem] inline-block object-cover"
        />
      )}
    </button>
  );
